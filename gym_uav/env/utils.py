...
def obs_dist_circle(obs_x, obs_y, uav_x, uav_y, end_x, end_y, r, obs_cnt):
    delta_y = end_y - uav_y
    delta_x = end_x - uav_x
    line_length = np.sqrt(delta_x**2 + delta_y**2)
    cos = delta_x / line_length
    sin = delta_y / line_length
    dist = line_length
    for i in range(obs_cnt):
        foot_x = (delta_y**2 * uav_x + delta_x**2 * obs_x[i] + (obs_y[i] - uav_y) * delta_x * delta_y) / line_length**2
        dist_to_line = np.abs(obs_y[i] - uav_y) if np.abs(sin) < 0.0001 else np.abs((obs_x[i] - foot_x) / sin)
        if dist_to_line <= r:
            d = np.sqrt(r**2 - dist_to_line**2)
            if np.abs(cos) < 0.0001:
                intersect_y1 = obs_y[i] - d
                intersect_y2 = obs_y[i] + d
                if ((intersect_y1 >= uav_y and intersect_y1 <= end_y) or
                    (intersect_y1 <= uav_y and intersect_y2 >= end_y)):
                    tmp_dist = np.abs(intersect_y1 - uav_y)
                    dist = min(tmp_dist, dist)
                if ((intersect_y2 >= uav_y and intersect_y2 <= end_y) or
                    (intersect_y2 <= uav_y and intersect_y2 >= end_y)):
                    tmp_dist = np.abs(intersect_y2 - uav_y)
                    dist = min(tmp_dist, dist)
            else:
                intersect_x1 = foot_x - d * cos
                intersect_x2 = foot_x + d * cos
                if ((intersect_x1 >= uav_x and intersect_x1 <= end_x) or
                    (intersect_x1 <= uav_x and intersect_x2 >= end_x)):
                    tmp_dist = np.abs((intersect_x1 - uav_x) / cos)
                    dist = min(tmp_dist, dist)
                if ((intersect_x2 >= uav_x and intersect_x2 <= end_x) or
                    (intersect_x2 <= uav_x and intersect_x2 >= end_x)):
                    tmp_dist = np.abs((intersect_x2 - uav_x) / cos)
                    dist = min(tmp_dist, dist)
    return dist
...
class RawEnv:
    ...
    def __init__(self, size_len, obs_interval, obs_percentage,
                 ...
                 alpha, beta, sigma, r_free, r_step,
                 sensor_max_dist):
        # assertions
    ...
    def act(self, action):
        ...
        self.agent_history_pos.append(self.agent_current_pos.copy())
        self.agent_current_pos[0] += math.cos(self.agent_orientation * math.pi) * self.dist_per_move
        self.agent_current_pos[1] += math.sin(self.agent_orientation * math.pi) * self.dist_per_move
    ...
    def _obs_sensor_observe(self, angle):
        ...
        dist = obs_dist_circle(obs_x, obs_y, cpos[0], cpos[1], end[0], end[1], self.obs_radius, len(obs_x))
        return dist
    ...
    def render(self) -> None:
        ...
        # 设置较小的窗口大小
        window_size = [800, 800]
        ratio = window_size[0] / self.size_len
        ...
        for his_pos in self.agent_history_pos:
            pygame.draw.circle(self.screen, (200, 200, 200),
                               (his_pos[0] * ratio, his_pos[1] * ratio), 2)
        pygame.draw.circle(self.screen, (0, 0, 0),
                           (self.agent_current_pos[0] * ratio, self.agent_current_pos[1] * ratio),
                           5)
        pygame.draw.circle(self.screen, (255, 0, 0),
                           (self.destination_pos[0] * ratio, self.destination_pos[1] * ratio), 10)
        pygame.display.flip()
