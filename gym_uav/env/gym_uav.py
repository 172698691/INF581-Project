...
class GymUav(gym.Env):
    def __init__(self, size_len=100.0, obs_interval=20.0,
                 ...
                 agent_initial_ori=0.25, sensor_max_dist=25.0, reward_params=None):
        if reward_params is not None:
            self.reward_params = reward_params
        else:
            self.reward_params = environ_config.reward_params
        # reward_params = [sigma, alpha, beta, r_free, r_step]
        sigma = self.reward_params[0]
        alpha = self.reward_params[1]
        beta = self.reward_params[2]
        r_free = self.reward_params[3]
        r_step = self.reward_params[4]
        self.raw_env = RawEnv(
            ...
            alpha, beta, sigma, r_free, r_step, sensor_max_dist
        )
