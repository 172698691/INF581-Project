...
from ddpg_pytorch.utils.nets import Actor, Critic
...
def torch_has_compile():
    return 'compile' in dir(torch)


class DDPG(object):

    def __init__(self, gamma, tau, hidden_size, num_inputs, action_space, device=None, checkpoint_dir=None):
        """
        ...
        self.do_compile = torch_has_compile()
        self.gamma = gamma
        self.tau = tau
        ...
        self.actor_target = Actor(hidden_size, num_inputs, self.action_space).to(self.device)

        # Define the critic
        self.critic = Critic(hidden_size, num_inputs, self.action_space).to(self.device)
        self.critic_target = Critic(hidden_size, num_inputs, self.action_space).to(self.device)
        ...
        # if self.do_compile:
        #     self._actor, self.actor = self.actor, torch.compile(
        #         self.actor, fullgraph=True, mode='reduce-overhead'
        #     )
        #     self._critic, self.critic = self.critic, torch.compile(
        #         self.critic, fullgraph=True, mode='reduce-overhead'
        #     )
        #     self._actor_target, self.actor_target = self.actor_target, torch.compile(
        #         self.actor_target, fullgraph=True, mode='reduce-overhead'
        #     )
        #     self._critic_target, self.critic_target = self.critic_target, torch.compile(
        #         self.critic_target, fullgraph=True, mode='reduce-overhead'
        #     )
        # else:
        self._actor = self.actor
        self._critic = self.critic
        self._actor_target = self.actor_target
        self._critic_target = self.critic_target
    ...
    def calc_action(self, state, action_noise=None):
        ...
        for i in range(len(mu[0])):
            mu[0][i] = mu[0][i].clamp(self.action_space.low[i], self.action_space.high[i])
    ...
    def save_checkpoint(self, last_timestep, replay_buffer, postfix='', override=False):
        """
        ...
        Arguments:
            ...
            postfix : postfix for save model
            override : override the file if it already exists, else print a warning and do nothing
        """
        assert self.checkpoint_dir is not None, 'cannot save, checkpoint dir is None'
        checkpoint_name = self.checkpoint_dir + '/ep_{}{}.pth.tar'.format(last_timestep, postfix)
        if os.path.exists(checkpoint_name):
            if not override:
                logger.warning('checkpoint already exists, not overriding')
                return False
            else:
                logger.info('checkpoint already exists, overriding')
